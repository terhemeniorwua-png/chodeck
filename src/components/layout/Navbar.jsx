...
function Flag({ src, alt }) {
  return (
    <img src={src} alt={alt} className="w-[20px] h-[20px]" loading="lazy" />
  )
}

function CountrySelector() {
  return (
    <span className="relative group/country">
      ...
      <span className="hidden group-hover/country:flex flex-col items-end w-full absolute top-[100%] right-0 pt-2 min-w-max">
        <button className="py-3 px-6 rounded-full bg-white border border-slate-200 mb-2 text-black font-semibold capitalize flex items-center space-x-2 cursor-pointer hover:bg-slate-50 transition-colors">
          <Flag src="/images/GhanaFlag.svg" alt="ghana" />
          <span className="capitalize">ghana</span>
        </button>
        <button className="py-3 px-6 rounded-full bg-white border border-slate-200 text-black font-semibold capitalize flex items-center space-x-2 cursor-pointer hover:bg-slate-50 transition-colors">
          <Flag src="/images/NigeriaFlag.svg" alt="nigeria" />
...
function ProductsDropdown() {
  const products = [
    { label: 'Chowpass', href: '/chowpass', align: '', hoverClass: 'hover:text-[#8C77EC] hover:border-[#8C77EC]' },
    { label: 'Relay', href: '/relay', align: 'ml-auto', hoverClass: 'hover:text-relay-main hover:border-relay-main' },
    { label: 'Ads', href: '/ads', align: 'ml-auto', hoverClass: 'hover:text-[#004699] hover:border-[#004699]' },
    { label: 'Chowstore', href: '/chowstore', align: 'ml-auto', hoverClass: 'hover:text-chow-pepper-100 hover:border-chow-pepper-100' },
  ]
      ...
      <div className="absolute top-0 pt-14 left-0 w-max">
        <div className="rounded-2xl transform origin-top transition-all duration-200 ease-out scale-y-0 group-hover:scale-y-100">
          <div className="hidden group-hover:flex flex-col p-4 space-y-3">
            {products.map(({ label, href, align, hoverClass }) => (
              <Link
                key={label}
                to={href}
                className={`py-2 px-5 rounded-full w-fit bg-white border border-slate-200 text-black font-semibold capitalize ${align} ${hoverClass} transition-all duration-200 transform hover:scale-105 hover:shadow-md`}
              >
                {label}
              </Link>
            ))}
  ...
  const mobileLinks = [
    ...
    { label: 'Sign In', href: 'https://chowdeck.com/store/sign-in', color: 'text-chow-green', icon: <FaBowlFood size={24} /> },
  ]
...
function MobileMenu({ open, onClose }) {
  return (
    <>
      ...
      />
      <div
        className={`fixed top-0 left-0 w-[85%] max-w-sm h-full bg-black z-[1000] overflow-y-auto transition-transform duration-300 ease-out ${
      ...
      >
        <div className="flex flex-col h-full">
          <div className="sticky top-0 left-0 w-full bg-black flex items-center justify-end space-x-2 pt-10 pb-6 border-b border-b-white/20">
            <a
              className="inline-flex items-center justify-center bg-white rounded-full w-12 h-12"
              target="_blank"
              rel="noreferrer"
              href="https://chowdeck.com/store"
            >
              <CartIcon />
            </a>
            <button
  ...
  return (
    <span className="hidden lg:flex lg:mr-2">
      <span className="relative block group">
        <button className="inline-flex items-center justify-center py-3 px-6 rounded-full bg-white text-black font-semibold capitalize">
          More
          <svg
            stroke="currentColor"
            fill="currentColor"
            ...
            <div className="hidden group-hover:flex flex-col py-4 space-y-3 px-2">
              {moreItems.map(({ label, href }) => (
                <Link
                  key={label}
                  to={href}
                  className="inline-flex items-center justify-center py-3 px-6 rounded-full ml-auto w-fit bg-white border border-slate-200 text-black font-semibold capitalize hover:bg-opacity-100 transition-all duration-200 transform hover:scale-105 hover:shadow-md hover:border-black"
                >
                  {label}
                </Link>
              ))}
...
function Navbar() {
  ...
  } w-full z-[500] transition-[top] duration-300 ease-out`}
>
        <nav className="container grid grid-cols-2 lg:grid-cols-5 xl:grid-cols-3 items-center h-20">
          <span className="col-span-1 flex items-center space-x-3">
            ...
            <span className="hidden xl:flex">
              <Link
                to="/"
                onClick={scrollToTop}
                className="bg-chow-green inline-flex items-center justify-center text-white py-3 px-6 rounded-full font-semibold h-[45px] transition-transform hover:scale-105"
              >
            ...
            <ul className="bg-white rounded-full flex items-center px-4 w-fit">
              <li>
                <NavLink
                  to="/company"
                  className={({ isActive }) =>
                    `py-4 px-6 text-center inline-block font-semibold ${
                      isActive ? 'text-chow-yellow' : ''
                    }`
                  }
                >
                  Company
              ...
              <li>
                <Link
                  to="/riders"
                  className="py-4 px-6 text-center inline-block font-semibold"
                >
                  Riders
                </Link>
              </li>
          ...
          <span className="flex items-center min-w-fit justify-end">
            ...
            <a
              ...
              className="hidden lg:inline-flex items-center gap-2 bg-white rounded-full px-3 h-12 mr-2 font-medium text-base active:scale-95 transition-transform shadow-sm"
              target="_blank"
              rel="noreferrer"
            >
              <CartIcon />
